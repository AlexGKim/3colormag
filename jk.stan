...
generated quantities {
  real residual;
  {
    real pv;                                 // peculiar velocity
    real pv0;
    real pz;                                 // peculiar redshift
    real pz0;
    ...
    // data prediction
    for (d in holdout:holdout){           //for each supernova pair
    // prediction for Delta
      residual =  fiveoverlog10 * pz/zcmb[d] - pz0term + dm_sig*dm_unit[d] - dm0term + dot_product(alpha_rescale, snparameters[d]);
    }
    residual = delta_holdout - residual;
  }
}
